...
func main() {
	...
	flag.StringVar(&leaderElectionID, "leader-election-id", "2eea155c.autoscaling.workload.io", "ID for leader election")
	flag.BoolVar(&secureMetrics, "metrics-secure", true,
		"If set, the metrics endpoint is served securely via HTTPS. Use --metrics-secure=false to use HTTP instead.")
	flag.BoolVar(&enableHTTP2, "enable-http2", false,
		"If set, HTTP/2 will be enabled for the metrics and webhook servers")
	flag.DurationVar(&timeoutDuration, "timeout", 30*time.Second, "The default reconcile timeout")
	opts := zap.Options{
		Development: true,
	}
	...
	}).SetupWithManager(mgr, timeoutDuration); err != nil {
