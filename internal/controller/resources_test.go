...
func TestRoundUp(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "Round up to nearest 100m",
			quantity: "150m",
			step:     "100m",
			expected: "200m",
		},
		{
			name:     "Round up to nearest 3m",
			quantity: "5m",
			step:     "3m",
			expected: "6m",
		},
		{
			name:     "Round up to nearest 0.1",
			quantity: "0.15",
			step:     "0.1",
			expected: "0.2",
		},
		{
			name:     "Round up to nearest 10",
			quantity: "95",
			step:     "10",
			expected: "100",
		},
		{
			name:     "Round up to nearest 1Ki",
			quantity: "1500",
			step:     "1024",
			expected: "2048",
		},
		{
			name:     "Round up to nearest 1Gi",
			quantity: "1.5Gi",
			step:     "1Gi",
			expected: "2Gi",
		},
		{
			name:     "Round up to nearest 0.01",
			quantity: "0.005",
			step:     "0.01",
			expected: "0.01",
		},
		{
			name:     "Exact step size",
			quantity: "200m",
			step:     "100m",
			expected: "200m",
		},
		{
			name:     "Round up to nearest 500Mi",
			quantity: "750Mi",
			step:     "500Mi",
			expected: "1000Mi",
		},
		{
			name:     "Round up to nearest 1",
			quantity: "3",
			step:     "1",
			expected: "3",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			quantity := resource.MustParse(tt.quantity)
			step := resource.MustParse(tt.step)
			expected := resource.MustParse(tt.expected)

			result := roundUp(quantity, step)
